...
class SSA(nn.Module):
    ...
    def forward(self, x):
        ...
        # 空间只有q使用可变性卷积
        # q = self.deformconv(x)
        # _, k_spa, v_spa = self.qkv(x).chunk(3, dim=1)

        # 空间q，k，b都使用可变性卷积
        q = self.deformconv(x)
        k_spa = self.deformconv(x)
        v_spa = self.deformconv(x)
        ...
        q_spa = rearrange(q, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
        k_spa = rearrange(k_spa, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
        v_spa = rearrange(v_spa, 'b (head c) h w -> b head c (h w)', head=self.num_heads)
        ...
        k_spa = torch.nn.functional.normalize(k_spa, dim=-1)
        v_spa = torch.nn.functional.normalize(v_spa, dim=-1)

        attn_spe = (q_spe @ k_spe.transpose(-2, -1)) * self.temperature
        attn_spe = attn_spe.softmax(dim=-1)
        attn_spa = (q_spa @ k_spa.transpose(-2, -1)) * self.temperature
        attn_spa = attn_spa.softmax(dim=-1)

        out_spe = (attn_spe @ v_spe)
        out_spa = (attn_spa @ v_spa)

        out_spe = rearrange(out_spe, 'b head c (h w) -> b (head c) h w', head=self.num_heads, h=h, w=w)
        out_spa = rearrange(out_spa, 'b head c (h w) -> b (head c) h w', head=self.num_heads, h=h, w=w)
        out = x + out_spe + out_spa
        out = self.tail(out)
