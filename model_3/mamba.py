...
from model_3.transformer import SA
...
class DWTMambaBlock(nn.Module):
    ...
    def __init__(self, dim, H, W):
        ...
        self.block_HL = Mamba(dim, expand=1, d_state=8, bimamba_type='v6',
                              if_devide_out=True, use_norm=True, input_h=H // 2, input_w=W // 2)
        self.block_LH = Mamba(dim, expand=1, d_state=8, bimamba_type='v6',
                              if_devide_out=True, use_norm=True, input_h=H // 2, input_w=W // 2)
        self.block_HH = Mamba(dim, expand=1, d_state=8, bimamba_type='v6',
                              if_devide_out=True, use_norm=True, input_h=H // 2, input_w=W // 2)
    ...
    def forward(self, input):
        ...
        # output_HL = self.block_HL(input_HL)
        # output_LH = self.block_HL(input_LH)
        # output_HH = self.block_HL(input_HH)
...
class SDMambaBlock(nn.Module):
    ...
    def __init__(self, dim, H, W, conv=default_conv):
        ...
        self.H = H
        self.W = W
        # self.ShiftMambaBlock = ShiftMambaBlock(dim, H, W)
