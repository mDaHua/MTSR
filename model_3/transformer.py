...
class SSA(nn.Module):
    ...
    def forward(self, x):
        ...
        #_, k_spa, v_spa = self.qkv2(x).chunk(3, dim=1)
        ...
        # q_spa = rearrange(q, 'b (head c) h w -> b head (h w) c', head=self.num_heads)
        # k_spa = rearrange(k_spa, 'b (head c) h w -> b head (h w) c', head=self.num_heads)
        # v_spa = rearrange(v_spa, 'b (head c) h w -> b head (h w) c', head=self.num_heads)
        ...
        # out_spa = rearrange(out_spa, 'b head (h w) c -> b (head c) h w', head=self.num_heads, h=h, w=w)

        out = out_spe
        # out = self.tail(out_spe+out_spa)
